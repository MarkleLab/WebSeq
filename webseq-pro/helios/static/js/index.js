...
$(document).ready(function() {
  ...
  $('#igvbutton').on('click', function() {
    ...
    data.append('trackfiles', trackFileList)
    storedFilesIGV.forEach(function(file, i) {
      data.append('igvfile', file);
    });
    console.log($('#genomeVersionID').find(":selected").text())
    data.append('genomeVersion',$('#genomeVersionID').find(":selected").text());
    // send ajax POST request to start background job
    $.ajax({
      type: 'POST',
      url: '/visualize',
...
$(document).ready(function() {
  ...
  $("#trackupload").on('change', function(e) {
    $('.alert-danger').css('display', 'none');

    trackFileList = []
    console.log("We're in here yee ")
    var files = e.target.files
    var filesArr = Array.prototype.slice.call(files);
    var input = document.getElementById('trackupload');
    var output = document.getElementById('trackFileList');
    output.innerHTML = '<ul>';
    // var urls = []
    for (var i = 0; i < input.files.length; ++i) {
      f = input.files.item(i);
      trackFileList.push(f.name);
      output.innerHTML += '<li>' + f.name + '</li>';
      if (['bam', 'cram'].indexOf(f.name.split('.')[f.name.split('.').length - 1]) < 0) {
        $('.alertFile').css('display', '');
      }
      ...
      console.log("In the update_progress methods..... ")
        ...
        if (data['state'] != 'PENDING' && data['state'] != 'PROGRESS') {
            if ('result' in data) {
                ...
                else if (pathname == '/fastqtovcf') {
                  $('.inprogress').css('display', 'none');
                  $('.completed').css('display', 'none');
                  $('#download_fastqtovcfbutton').css('display', 'inline');
                  $('#push_fastqtovcfbutton').css('display', 'inline');
                }

                else if (pathname == '/gene'){
                  $('.inprogess').css('display', 'none');
                  $('.completed').css('display', 'inline');
                  $('#download_genepanelbutton').css('display', 'inline');
                }
...
$(document).ready(function(){
  ...
  var pathname = window.location.pathname;
  if (pathname == "/fastqtovcf"){

    var selDiv = "";
    var storedFiles = [];

    var form = document.getElementById("fastqtovcfform");
    console.log(form)
    $('#download_fastqtovcfbutton').css('display', 'none');
    $('#push_fastqtovcfbutton').css('display', 'none')

    $('.completed').css('display', 'none');
    $('.custommargin h6').css('display', 'inline');

    $("div #fastqtovcfbutton").on('click', function(){
      $('#download_fastqtovcfbutton').css('display', 'none')
      $('#push_fastqtovcfbutton').css('display', 'none')
      $('.completed').css('display', 'none');
      $('.notstarted').css('display', 'none');
      ...
      console.log($('#fastqtovcfform'))
      var form_data = new FormData($('#fastqtovcfform')[0]);
     ...
     data.append('readgrouptext',$('#readgrouptext')[0].value)

     console.log(data)
      // send ajax POST reques to start background job
      $.ajax({
        type: 'POST',
        url: '/fastqtovcf',
        data: data,
        ...
        success: function(data, status, request){
          ...
          console.log("AJAX call returned from Fastq to VCF")
    ...
    $("div #download_fastqtovcfbutton").on('click', function(){
      $.ajax({
        type: 'POST',
        url: '/download_fastqtovcf',
        success:function(data, status, request) {
        }
      });
    });

    // $("div #push_fastqtovcfbutton").on('click', function() {
    //
    //   // var data = new FormData();
    //   // data.append('url', "fastq");
    //   // $.ajax({
    //   //   type: 'POST',
    //   //   url: '/push',
    //   //   async:false,
    ...
    $("body").on('click', '.fastqtovcficon', function(e){
      var file = $(e.currentTarget)["0"].parentNode.innerText
      console.log(file)
    ...
    $("#fastqtovcfupload").on('change', function(e) {
      $('.alert-danger').css('display', 'none');

      var files = e.target.files
      var filesArr = Array.prototype.slice.call(files);
      var input = document.getElementById('fastqtovcfupload');
      var output= document.getElementById('fastqtovcfFileList');

      for (var i = 0; i < input.files.length; ++i) {
        f = input.files.item(i);
        storedFiles.push(f);
        output.innerHTML += '<li>' + f.name + '<i class= "fas fa-trash-alt fastqtovcficon"></i>' + '</li>';

        if (['fastq.gz'].indexOf(f.name.split('.')[f.name.split('.').length - 1]) < 0){
          $('.alert-danger').css('display', '');
        }
      }
    });
...
$(document).ready(function(){
  ...
  if (pathname == "/gene"){
    ...
    var form = document.getElementById("genepanelform");
    $('#download_genepanelbutton').css('display', 'none');
    $('#push_genepanelbutton').css('display', 'none')

    $('.completed').css('display', 'none');
    $('.custommargin h6').css('display', 'inline');

    $("div #genepanelbutton").on('click', function(){
      $('#download_genepanelbutton').css('display', 'none')
      $('#push_genepanelbutton').css('display', 'none')
      $('.completed').css('display', 'none');
      $('.notstarted').css('display', 'none');
      $('.inprogess').css('display', 'inline');

      console.log("button clicked")

      var form_data = new FormData($('#genepanelform')[0]);
      var data = new FormData();
      ...
      $.ajax({
        ...
        success: function(data, status, request){
          ...
          console.log("AJAX call returned from genepanel")
          status_url = request.getResponseHeader('Location');
          update_progress(status_url);
    //      $('.inprogess').css('display', 'none');
          console.log(data)
    ...
    $("div #download_genepanelbutton").on('click', function() {
      $.ajax({
        type: 'POST',
        url: '/downloadgenepanel',
        success: function(data, status, request) {
          length = data.length
          for (i = 0; i < length; i++) {
            file_data = data[i][1]
            file_name = data[i][0]
            parsed_data = Papa.unparse(file_data)
            download(parsed_data, file_name, "csv")
          }
        }
    ...
    $("div #push_genepanelbutton").on('click', function(){
      $.ajax({
        type:'POST',
        url: '/push',
        async: false,
    ...
    $("body").on('click', '.genepanelicon', function(e) {
      var file = $(e.currentTarget)["0"].parentNode.innerText
      console.log(file)
      for (var i = 0; i < storedFiles.length; i++){
        if (storedFiles[i].name.trim() == file.trim()) {
    ...
    $("#genepanelupload").on('change', function(e) {
      $('.alert-danger').css('display', 'none');

      var files = e.target.files
      var filesArr = Array.prototype.slice.call(files);
      var input = document.getElementById('genepanelupload');
      var output = document.getElementById('genepanelFileList');

      for (var i = 0; i < input.files.length; ++i) {
        f = input.files.item(i);
        storedFiles.push(f);
        output.innerHTML += '<li>' + f.name + '<i class= "fas fa-trash-alt genepanelicon"></i>' + '</li>';

        if (['csv'].indexOf(f.name.split('.')[f.name.split('.').length-1])< 0) {
          $('.alert-danger').css('display', '');
        }
      }
...
$(document).ready(function() {
  ...
  if (pathname == "/onevcf"){
      ...
      if (push_length != 0 && $any_push_files != "") {
        var output = document.getElementById('onevcfPushFileList');
